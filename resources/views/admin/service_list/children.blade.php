...
@section('content')
    <div class="col-sm-6">
        ...
        <table class="table" id="mytable">
            ...
            <tr>
                <th class="text-right">Agency Unit</th>
                <td>{{ $detail->agency->parent->agency_unit_name }}</td>
            </tr>
            <tr>
                <th class="text-right">Service Unit</th>
                <td>{{ $detail->agency->agency_unit_name }}</td>
            </tr>
            <tr>
        ...
        @foreach ($detail->children as $child)
            <div class="col-sm-12">
                <table class="table">
                    <tr>
                        <td style="width: 60%">
                            <h5>{{ $child->service_name }}</h5>
                        </td>
                        <td align="right">
                            <span class="text-right">
                                <a href="{{ route('workflows.add', ['id_service' => $child->id_service]) }}"
                                    class="btn btn-md btn-success" title='Add workflow'><i class='fa fa-plus'></i> Add
                                    Workflow</a> &nbsp;
                                <a href="{{ route('member.pricelist.index', ['id_service' => $child->id_service]) }}"
                                    class="btn btn-md btn-success" title='Manage Pricelist'><i
                                        class='fa fa-money-bill-alt'></i> Pricelist </a> &nbsp;
                                <a href="#" onClick="editService({{ $child->id_service }})"
                                    class="btn btn-md btn-warning"><i class='fa fa-edit'></i> Edit</a> &nbsp;
                                <a href="#" onClick="deleteService({{ $child->id_service }})"
                                    class="btn btn-md btn-danger"><i class='fa fa-trash'></i> Delete</a>
                            </span>
                        </td>
                    </tr>
                </table>
                        ...
                        <tbody>
                            ...
                            @foreach ($children as $flow)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('workflows.edit', [$flow->id_service_workflow]) }}"
                                            class="btn btn-sm btn-warning"><i class='fa fa-edit'></i></a> &nbsp;
                                        <a href="#" onClick="deleteWorkflow({{ $flow->id_service_workflow }})"
                                            class="btn btn-sm btn-danger"><i class='fa fa-trash'></i></a> &nbsp;
                                    </td>
                                    <td>
                                        <a href="{{ route('workflow.docs', ['id_service_workflow' => $flow->id_service_workflow]) }}"
                                            class="btn btn-sm btn-success" title="Add Workflow doc"><i
                                                class='fa fa-file'></i></a> &nbsp;
                                        <a href="{{ route('workflow.infos', ['id_service_workflow' => $flow->id_service_workflow]) }}"
                                            class="btn btn-sm btn-success" title="Add workflow info"><i
                    ...
                    <div class="modal-body">
                        <form id="frmService">
                            <div class="form-group">
                                ...
                                <input type="hidden" name="id_service_parent" id="id_service_parent"
                                    value="{{ $detail->id_service }}" class="form-control">
    ...
    <script type="text/javascript">
        function deleteWorkflow(id) {
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = "{{ route('workflows.delete_child', ':id') }}".replace(':id', id);
                }
            });
        }
        ...
        function deleteService(id) {
            swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "{{ route('services_api.delete', ':id') }}".replace(':id', id),
                        type: 'GET',
                        ...
                        error: function() {
                            swal.fire('ERROR!', 'Service can not be removed.', 'error');
                        }
                    });
                }
            });
