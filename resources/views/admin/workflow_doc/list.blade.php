...
@section('content')
  ...
  <p>
    <a href="{{ route('workflow_docs.add', ['id_service_workflow' => \Request()->id_service_workflow]) }}" class="btn btn-success"><i class='fa fa-plus'></i> Add Data</a>
    ...
    <tbody>
      ...
      @foreach($list as $l)
        <?php 
          ...
          if ($total > 1){
            if ($no == $total ) {
              $action .= "<a href='".route('workflow_docs.sequence.update', ['id_service_workflow' => \Request()->id_service_workflow, 'id' => $l->id_service_workflow_doc, 'arrow' => 'up'])."'><i class='fa fa-arrow-up'></i></a> ";
            }elseif ($no == 1 ) {
              $action .= "<a href='".route('workflow_docs.sequence.update', ['id_service_workflow' => \Request()->id_service_workflow, 'id' => $l->id_service_workflow_doc, 'arrow' => 'down'])."'><i class='fa fa-arrow-down'></i></a> ";
            }else{
              $action .= "<a href='".route('workflow_docs.sequence.update', ['id_service_workflow' => \Request()->id_service_workflow, 'id' => $l->id_service_workflow_doc, 'arrow' => 'up'])."'><i class='fa fa-arrow-up'></i></a> ";
              $action .= "<a href='".route('workflow_docs.sequence.update', ['id_service_workflow' => \Request()->id_service_workflow, 'id' => $l->id_service_workflow_doc, 'arrow' => 'down'])."'><i class='fa fa-arrow-down'></i></a> ";
            }
        ...
        <tr>
          ...
          <td>{{ $l->description }}</td>          
          <td>
            <a href="{{ route('workflow_docs.edit', [$l->id_service_workflow_doc]) }}" class="btn btn-sm btn-warning"><i class='fa fa-edit'></i></a> &nbsp;
            <a href="#" class="btn btn-sm btn-danger" onclick="deleteRow({{ $l->id_service_workflow_doc}})"><i class='fa fa-trash'></i></a> &nbsp;
          </td>          
        </tr>
      @endforeach
    </tbody>
  ...
  <script type="text/javascript">
    ...
    function deleteRow(id){
      swal.fire({
        ...
        if (result.value) { 
          $.ajax({
            url: "<?=URL::to('admin/workflow_docs');?>" + '/' + id,
