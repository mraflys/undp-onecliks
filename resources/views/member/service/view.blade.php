...
@section('content')
    ...
    <table cellpadding="5" class="table table-striped">
        ...
        <tr>
            ...
            <tr>
                ...
                <td>{{ $detail->currency ? $detail->currency->currency_name : 'USD' }}
                    {{ $detail->service_price * $detail->qty }}</td>
            </tr>
            <!-- payment method -->
            <tr>
                                ...
                                @foreach ($comments as $comment)
                                    <tr>
                                        <td>
                                            ...
                                            <hr>
                                            <b>{{ $comment->created_by }}</b> comment at
                                            <small>{{ $comment->date_created }}</small> <br><br>
                                            <i>{{ $comment->comment }}</i>
                                        </td>
                                    </tr>
                                @endforeach
                ...
                <td>
                    @php
                        ...
                        foreach ($workflows as $workflow) {
                            if ($workflow->infos && count($workflow->infos) > 0) {
                                $allInfos = $allInfos->merge($workflow->infos);
                            }
                        }
                    @endphp
                    @if (count($allInfos) > 0)
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                @foreach ($allInfos as $info)
                                    @if (isset($workflows[0]) && $workflows[0]->id_transaction_workflow == $info->id_transaction_workflow)
                                        <tr>
                                            <td>{{ $count }}</td>
            ...
            <tr>
                ...
                <td>
                    @php
                        $allDocs = collect();
                        foreach ($workflows as $workflow) {
                            if ($workflow->docs && count($workflow->docs) > 0) {
                                $allDocs = $allDocs->merge($workflow->docs);
                            }
                        }
                    @endphp
                    @if (count($allDocs) > 0)
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                @foreach ($allDocs as $doc)
                                    @if (isset($workflows[0]) && $workflows[0]->id_transaction_workflow == $doc->id_transaction_workflow)
                                        <tr>
                                            <td>{{ $count }}</td>
                                            <td>{{ $doc->document_name }}</td>
                                            <td><a href="{{ $doc->document_path }}" target="_blank"><i
    ...
    <div class="row">
                            ...
                            <div class="card-body">
                                <table class="table">
                                    <tr>
                                        ...
                                        <td>Actual Day (s) :
                                            <?= $workflow->date_end_actual == null ? date('Y-m-d H:i:s') : '' ?></td>
                                        <?php
                                        ...
                                        <td>
                                            ...
                                            {{ $pic != null ? $pic->person_name . ' - ' . $agency_unit_name : '-' }}
                                            <br>Completed By:
                                            {{ $finisher != null ? $finisher->person_name . ' - ' . $finisher_agency_unit_name : '' }}
                                        </td>
                                    </tr>
                                </table>
                            </div>
        ...
        <div class="col-sm-12">
            <hr>
            <p class="text-center"><a href="{{ route('myservices.index') }}" class="btn btn-info"><i
                        class="fa fa-arrow-left"></i> Back</a></p>
        </div>
    </div>
@endsection
