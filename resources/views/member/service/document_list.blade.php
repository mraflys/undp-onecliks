...
<script type="text/javascript">
    function document_list(id) {
        ...
        $.ajax({
            ...
            success: function(data) {
                console.log(data[0]);
                $("#loadingProject").html(
                    "<?= \App\GeneralHelper::message_dismissable('success', 'Project have been Open') ?>"
                    )
                const dataLength = data.length;
                var html = '';
                for (let dataStart = 0; dataStart < dataLength; dataStart++) {
                    html += '<tr><td>' + data[dataStart].name_workflow + '</td><td>' + data[dataStart]
                        .name_service + '</td><td><a href="' + data[dataStart].document_url +
                        '" target="_blank">' + data[dataStart].document_name + '</a></td></tr>';
                }
                $('#listDocUl').html(html);
            },
            error: function(data) {
    ...
    function showTableDocument() {
        ...
        $('#mytableDocument_filter input').unbind();
        $('#mytableDocument_filter input').bind('keyup', function(e) {
        ...
        if ($('#mytableDocument_filter .btn-search-dt').length == 0) {
            $('#mytableDocument_filter').append(
                '&nbsp;<button type="button" class="btn btn-sm btn-primary btn-search-dt"><i class="fa fa-search"></i></button>'
            );
            $('#mytableDocument_filter .btn-search-dt').on('click', function() {
                oTable.search($('#mytableDocument_filter input').val()).draw();
            });
        }
    }
