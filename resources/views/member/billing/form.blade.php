...
<script type="text/javascript">
  $(function(){
    ...
    $('#id_agency_unit').change(function(){
      $.ajax({
        url: "<?=URL::to('member-area/billing/data/transaction_ready_to_bill');?>" +"/"+ $("#id_agency_unit").val(),
        dataType: 'json',
        beforeSend: function(){
          $("#transaction_ready_to_bill tbody").html("<tr><td colspan='7'>Loading.... Please wait a moment</td></tr>");
        },
        success: function(data){
          ...
          $.each(data.data, function(k, value){
            trRow += "<tr>";
            trRow += "<td><input type='checkbox' class='ids' name='ids["+value.id_transaction+"]' value='"+value.id_transaction+"' checked></td>";
            trRow += "<td>"+value.transaction_code+"</td>";
            trRow += "<td>"+value.service_name+"</td>";
            trRow += "<td style='max-width: 250px'>"+value.description+"</td>";
            trRow += "<td>"+value.payment_method+"</td>";
            trRow += "<td><input type='hidden' name='prices["+value.id_transaction+"]' value='"+value.service_price+"'>"+value.service_price+"</td>";
          });
