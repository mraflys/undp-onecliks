...
class ReportQuery
{
    ...
    {
        $query = "SELECT ag.id_agency_unit, ag.agency_unit_code, ag.agency_unit_name, ag.id_status, ag.status_name,
                         COALESCE(COUNT(t.id_transaction), 0) AS cnt,
                         COALESCE(SUM(t.value), 0) AS value,
                         total_cnt.total_transactions
        ...
        LEFT JOIN
          ( SELECT id_agency_unit_service, COUNT(DISTINCT id_transaction) AS total_transactions
            FROM (SELECT *
                  FROM tr_service
                  WHERE date_deleted IS NULL AND
                        id_transaction_parent IS NULL AND $where
                  ) tr_count
            GROUP BY id_agency_unit_service
          ) total_cnt ON total_cnt.id_agency_unit_service=ag.id_agency_unit
        GROUP BY ag.id_agency_unit, ag.agency_unit_code, ag.agency_unit_name, ag.id_status, ag.status_name, total_cnt.total_transactions
        ORDER BY COALESCE(total_cnt.total_transactions, 0) DESC, ag.agency_unit_code ASC, ag.id_status ASC";
