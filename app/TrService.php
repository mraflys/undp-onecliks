...
class TrService extends Model
{
  ...
  public static function history_search($filters, $additional_where = null){
    ...
    if (isset($filters['id_service_unit'])) $where .= " AND tr_service.id_agency_unit_service =".$filters['id_service_unit'];
    if (isset($filters['id_agency_unit_service'])) $where .= " AND tr_service.id_agency_unit_service IN (".$filters['id_agency_unit_service'].")";
    if (isset($filters['id_status'])) $where .= " AND tr_service.id_status IN (".$filters['id_status'].")";
    if (isset($filters['rating']) && $filters['rating'] > 0) $where .= " AND tr_service.service_rating IN (".$filters['rating'].")";
    if (isset($filters['with_rating_only'])) $where .= " AND (tr_service.date_rating IS NOT NULL OR tr_service.id_status=5)";
  ...
  public static function tracking($filters = array()) {
