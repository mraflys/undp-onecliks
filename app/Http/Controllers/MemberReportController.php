...
class MemberReportController extends Controller
{
  ...
  function get_critical($data){
    ...
    if (count($where) > 0) $where = implode(" OR ", $where);
    return ReportQuery::critical($where);
  }

  function get_formatted_critical_data($results){
    $label[2] = "recruitment";
  ...
  public function service_cost_post(Request $req){
    ...
    $where = implode(" AND ", $where);
    $color = array("#FF0000", "#0000FF", "#00FF00", "#FFFF00", "#FF00FF", "#00FFFF", "pink", "bluesky", "lightgreen");
    ...
    if (!is_null($income)){
      ...
      foreach($income as $row) {
        if (!isset($total_income_workday[$row->id_transaction])) $total_income_workday[$row->id_transaction] = 0;
    ...
    if(!empty($expenditure)) {
      ...
      $data["expenditure"] = str_replace(array("\"#", "#\""), "", json_encode($data_expenditure));
    }
    ...
    if (!empty($agency_expenditure)){
      ...
      $data["agency_expenditure"] = str_replace(array("\"#", "#\""), "", json_encode($data_agency_expenditure));
    }
  ...
  public function complete_ticket_list(Request $req){
    ...
    if (!is_null($date1)) $where[] = "DATE_FORMAT(tr.date_finished, '%Y-%m-%d') >= '".$date1."'";
    if (!is_null($date2)) $where[] = "DATE_FORMAT(tr.date_finished, '%Y-%m-%d')  <= '".$date2."'";
  ...
  public function coa_list(Request $req){
    ...
    if($req->viewBy == 'date_finished'){
        if (!is_null($date1)) $where[] = "DATE_FORMAT(".$req->viewBy.", '%Y-%m-%d') >= '".$date1."'";
        if (!is_null($date2)) $where[] = "DATE_FORMAT(".$req->viewBy.", '%Y-%m-%d')  <= '".$date2."' AND date_authorized IS NOT NULL";
    }elseif($req->viewBy == 'date_authorized'){
        if (!is_null($date1)) $where[] = "DATE_FORMAT(".$req->viewBy.", '%Y-%m-%d') >= '".$date1."'";
        if (!is_null($date2)) $where[] = "DATE_FORMAT(".$req->viewBy.", '%Y-%m-%d')  <= '".$date2."' AND date_finished IS NULL";
    }
  ...
  public function workload_analysis_list(Request $req){
    ...
    if (!is_null($date1)) $where[] = "DATE_FORMAT(date_start_actual, '%Y-%m-%d') >= '".$date1."'";
    if (!is_null($date2)) $where[] = "DATE_FORMAT(date_start_actual, '%Y-%m-%d')  <= '".$date2."'";
  ...
  public function dsa_advance_list(Request $req){
    ...
    if ($req->status > 0) $where[] = "tr.id_status = ".$req->status;
    if (!is_null($date1)) $where[] = "DATE_FORMAT(STR_TO_DATE(ret_date.info_value, '%Y-%m-%d'), '%Y-%m-%d') >= '".$date1."'";
    if (!is_null($date2)) $where[] = "DATE_FORMAT(STR_TO_DATE(ret_date.info_value, '%Y-%m-%d'), '%Y-%m-%d')  <= '".$date2."'";
  ...
  public function invoice_issue_list(Request $req){
    ...
    if (!is_null($date1)) $where[] = "DATE_FORMAT(date_created, '%Y-%m-%d') >= '".$date1."'";
    if (!is_null($date2)) $where[] = "DATE_FORMAT(date_created, '%Y-%m-%d')  <= '".$date2."'";
  ...
  public function user_registration_list(Request $req){
    ...
    $where = [];
    if (!empty($name)) $where[] = "person_name LIKE '%".$name."%'";
    if (!is_null($status) && $status != 'all') $where[] = "sec_user.is_active =".$status;
  ...
  public function timeliness_detail_list(Request $req){
    ...
    $date2 = ($req->date2) ? date(DATE_ONLY, strtotime($req->date2)) : $default_date2->format(DATE_ONLY);
    $where = [];
    if (!is_null($date1)) $where[] = "DATE_FORMAT(date_finished, '%Y-%m-%d') >= '".$date1."'";
    if (!is_null($date2)) $where[] = "DATE_FORMAT(date_finished, '%Y-%m-%d')  <= '".$date2."'";
  ...
  public function search_engine_list(Request $req){
    ...
    if (!empty($search_in)) {
      if ($search_in != "all") $where2 = " AND kode = '".$search_in."'";
    }
