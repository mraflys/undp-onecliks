...
class ServiceListController extends Controller
{
    ...
    public function search_by()
    {
        ...
        if (isset($_GET['parent_only'])) {
            $where .= " AND (ms_service.id_service_parent IS NULL OR ms_service.id_service_parent = '')";
        }

        if (isset($_GET['id_parent'])) {
            $where .= ' AND ms_service.id_agency_unit = ' . $_GET['id_parent'];
        }
    ...
    public function show_as_json(Request $req)
    {
        ...
        if (! is_null($service)) {
            ...
            if (! is_null($active_price)) {
                $active_price->currency;
            }
