...
class MemberRequestController extends Controller{
  ...
  function store_coa($req, $id_transaction, $table_type = 'tr_service'){
    try{
  ...
  function store_workflows($req, $tr_service, $id, $table_type = 'tr_service'){
    try{
      $total_day = 0;
      $sequence = 1;
      ...
      foreach ($workflows as $key => $value) {
        ...
        if (in_array($current_workflow->id_service, $id_service_insert))
        {
          ...
          if ($this->table == 'tr_service'){
            ...
            if ($sequence == 1) $tr_workflow["date_start_actual"] = $start_date;
  ...
  function store_workflow_info($infos, $workflow_parent_ids) {
    try {
      if (is_null($infos) || count($infos) < 1) return null;
      $workflow_info_ids = [];
      foreach ($infos as $key => $value) {
  ...
  function store_workflow_doc($docs, $workflow_parent_ids, $tableType, $id, $request) {
    try {
      ...
      if($tableType == 'tr_service'){
        if (is_null($docs) || count($docs) < 1) return null;
        $workflow_doc_ids = [];
...
//                var_dump($tr_workflow);
  ...
  public function ongoing_request_search(Request $req){
    ...
    ->editColumn('service_name', function($list){ 
      ...
      if (!empty($list->comment) && $list->id_status == -1) $desc .= "<br>comment: ".$list->comment;
      return $desc; 
    })
  ...
  public function old_ongoing_request_search(Request $req){
    ...
    ->editColumn('service_name', function($list){ 
      $desc = $list->service_name.' - '.$list->description;
      if (!empty($list->comment) && $list->id_status == -1) $desc .= "<br><b>Return comment: </b><br>".$list->comment;
      return $desc;
    })
    ->addColumn('action', function($list){
      $action = "";
