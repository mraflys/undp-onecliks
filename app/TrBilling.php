...
class TrBilling extends Model
{
    ...
    public static function transaction_ready_to_bill($where = "")
    {
        ...
        // Optimized query using LEFT JOIN instead of NOT IN subquery
        $query = "
            SELECT
                p.id_agency_unit,
                p.agency_unit_code,
    ...
    public static function agency_to_bill($where)
    {
        ...
        $query = "SELECT
            p.id_agency_unit,
            p.agency_unit_name,
            SUM(tr.service_price) AS service_price
        FROM tr_service tr
        JOIN ms_agency_unit a
            ON a.id_agency_unit = tr.id_agency_unit_buyer
        JOIN ms_agency_unit p
            ON p.id_agency_unit = a.id_agency_unit_parent
        WHERE
            tr.id_transaction_parent IS NULL
            AND tr.date_deleted IS NULL
            AND tr.is_free_of_charge = 0
            AND tr.date_start_billing IS NULL
            AND tr.service_price > 0
